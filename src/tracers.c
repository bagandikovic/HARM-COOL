...
#ifdef MPI_USED
    MPI_Sendrecv(_p, 1, slice_iNPR1_start, rank_previ, 123,
                 _p, 1, slice_iNPR1_stopN, rank_nexti, 123,
...
#ifdef MPI_USED
    MPI_Allreduce(MPI_IN_PLACE,&cnt,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD);
#endif
    if(rank == 0)
...
void Tracers_Sendrecv(std::vector<struct of_tracer> &vectr, int rank_to, int rank_from)
{
    ...
    MPI_Sendrecv(&trsend, 1, MPI_INTEGER, rank_to, 128,
                 &trrecv, 1, MPI_INTEGER, rank_from, 128,
                 MPI_COMM_WORLD, &MPIStatus);
//	printf("\nTracers_Sendrecv send %d to %d, recv %d from %d\n", trsend, rank_to, trrecv, rank_from);

    if(trsend > 0)
        MPI_Send(&vectr[0], trsend*sizeof(struct of_tracer), MPI_BYTE, rank_to, 129, MPI_COMM_WORLD);

    if(trrecv > 0) {
        int cursize = vtr.size();
        vtr.resize(cursize + trrecv);

        MPI_Recv(&vtr[cursize], trrecv*sizeof(struct of_tracer), MPI_BYTE, rank_from, 129, MPI_COMM_WORLD, &MPIStatus);
    }
}
#endif
...
#ifdef MPI_USED
    MPI_Sendrecv(_p, 1, slice_iNPR1_start, rank_previ, 126,
                 _p, 1, slice_iNPR1_stopN, rank_nexti, 126,
                 MPI_COMM_WORLD, &MPIStatus);
#if	N3>2
    {
        MPI_Sendrecv(_p, 1, slice_hNPR1_start, rank_prevh, 127,
                     _p, 1, slice_hNPR1_stopN, rank_nexth, 127,
                     MPI_COMM_WORLD, &MPIStatus);
    }
#endif
