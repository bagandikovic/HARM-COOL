...
void init()
{
    ...
    /* for disk interior */
    double l,rin,lnh,l2,rin2,lnh2,expm2chi,up1 ;
    double DD,AA,SS,thin,sthin,cthin,DDin,AAin,SSin,DDin2,AAin2,SSin2 ;
    double kappa,hm1 ;

    /* for magnetic field */
//	double A[N3+2][N1+4][N2+4] ;
    double rho_av,rhomax,umax,beta,bsq_ij,bsq_max,norm,q,beta_act ;
    double rmax, rmax2, lfish_calc(double rmax) ;
    ...
    fprintf(stderr, "our disks will have: rin1=%g, l1=%g and rin2=%g, l2=%g  \n", rin, l, rin2, l2);
...
//	ZSLOOP(0,N1-1,0,N2-1) {
    ZLOOP {
        ...
        DDin2 = rin2*rin2 - 2.*rin2 + a*a ;
        AAin2 = (rin2*rin2 + a*a)*(rin2*rin2 + a*a) - DDin2*a*a*sthin*sthin ;
        SSin2 = rin2*rin2 + a*a*cthin*cthin ;

        if(r >= rin) {
            lnh = 0.5*log((1. + sqrt(1. + 4.*(l*l*SS*SS)*DD/(AA*sth*AA*sth)))/(SS*DD/AA))
            - 0.5*sqrt(1. + 4.*(l*l*SS*SS)*DD/(AA*AA*sth*sth)) - 2.*a*r*l/AA
            - (0.5*log((1. + sqrt(1. + 4.*(l*l*SSin*SSin)*DDin/(AAin*AAin*sthin*sthin)))/(SSin*DDin/AAin))
            - 0.5*sqrt(1. + 4.*(l*l*SSin*SSin)*DDin/(AAin*AAin*sthin*sthin)) - 2.*a*rin*l/AAin ) ;
        }
        else
            lnh = -1. ;

        if(r >= rin2) {
            lnh2 = 0.5*log((1. + sqrt(1. + 4.*(l2*l2*SS*SS)*DD/(AA*sth*AA*sth)))/(SS*DD/AA))
            - 0.5*sqrt(1. + 4.*(l2*l2*SS*SS)*DD/(AA*AA*sth*sth)) - 2.*a*r*l2/AA
            - (0.5*log((1. + sqrt(1. + 4.*(l2*l2*SSin2*SSin2)*DDin2/(AAin2*AAin2*sthin*sthin)))/(SSin2*DDin2/AAin2))
            - 0.5*sqrt(1. + 4.*(l2*l2*SSin2*SSin2)*DDin2/(AAin2*AAin2*sthin*sthin)) - 2.*a*rin2*l2/AAin2 ) ;
        }
        else
            lnh2 = -1. ;
        ...
        if( (lnh < SMALL && lnh2 < SMALL)  || r < rin || cth > 0.7 || cth < -0.7) {
            rho = 1.e-7*RHOMIN ;
            u = 1.e-7*UUMIN ;
         ...
         * so it needs to be transformed at the end */
            ...
            ur = 0. ;
            uh = 0. ;
            /* calculate u^phi 'changed'*/
            expm2chi = SS*SS*DD/(AA*AA*sth*sth) ;
            up1 = sqrt((-1. + sqrt(1. + 4.*l*l*expm2chi))/2.) ;
            up = (2.*a*r*sqrt(1. + up1*up1)/sqrt(AA*SS*DD) + sqrt(SS/AA)*up1/sth) ;
            }

            if( lnh2 > SMALL )  // 2nd torus
            { 
            hm1 = exp(lnh2) - 1. ;
            rho = pow(hm1*(gam - 1.)/(kappa*gam), 1./(gam - 1.)) ;
            u = kappa*pow(rho,gam)/(gam - 1.) ;
            ...
            /* calculate u^phi */
            expm2chi = SS*SS*DD/(AA*AA*sth*sth) ;
            up1 = sqrt((-1. + sqrt(1. + 4.*l2*l2*expm2chi))/2.) ;
            up = -(2.*a*r*sqrt(1. + up1*up1)/sqrt(AA*SS*DD) + sqrt(SS/AA)*up1/sth) ;
            }
...
//        ZSLOOP(0,N1,0,N2) {
    ...
    // A[h][i][j] = 0.003*256*( 1 -sqrt(pow(cos(th),2)) );

    /* Uniform (vertical) field  */
     A[h][i][j] = 0.003*pow(r*sin(th),2);
     //A[h][i][j] = SMALL*pow(r*sin(th),2);

    /* Uniform for two torii(changed)*/    
    //A[h][i][j] = 0.003*pow(r*sin(th),2);

    /* Loop */
    // A[h][i][j] = 0.003*16384*sqrt(2)*pow(r*sin(th),2)/pow((16*16+r*r),1.5) ; // loop radius = 16
    
    /* Parabolic field */ // Tchekhovskoy et al. (2008); Nakamura et al. (2018):
    // A[h][i][j] = 0.003*(128/5)*pow(r,0.75)*( 1 - sqrt(pow(cos(th),2)) );


    // old
    /* Uniform (vertical) field  */ // plus and minus configuration are giving the same result
    // A[i][j] = 0.01*0.004607*pow(r*sin(th),2);
    // A[i][j] = 0.5*0.005*pow(r*sin(th),2);
    // A[i][j] = 0.004607*pow(r*sin(th),2); // old version 4-potential ???
  
    //  pro clanek do RAGtime proc. a = 0 (see coordinates.c)
    // A[i][j] = 0.003*pow(r*sin(th),2); // uniform (beta around 100?)
    // A[i][j] = 93.13*pow(r*sin(th),2)/pow((14*14+r*r),1.5) ; // loop R=14

    /* Monopole field */  
        

    // A[i][j] = 0.4607*144*( 1 -sqrt(pow(cos(th),2)) ) ; // STRONG
    // A[i][j] = 0.00004607*144*( 1 -sqrt(pow(cos(th),2)) ) ; // WEAK
    

    /* Dipole field */
    // A[i][j] = 0.5*sin(th)*sin(th)/r; 

    /* Loop - check it! */
    // A[i][j] = 0.004607*72*(sqrt(2)/3)*144*pow(r*sin(th),2)/pow((12*12+r*r),1.5) ; // loop radius = 12
    // A[i][j] = 0.4607*72*(sqrt(2)/3)*144*pow(r*sin(th),2)/pow((12*12+r*r),1.5) ; // STRONG
    // A[i][j] = 0.00004607*72*(sqrt(2)/3)*144*pow(r*sin(th),2)/pow((12*12+r*r),1.5) ; // WEAK  

    /* Beskin disk - OK */
    // A[i][j] = (1 - sqrt((1 - pow(r,2)/100 )/2 + sqrt(pow(1 - pow(r,2)/100 ,2)/4 + (pow(r,2)*pow(cos(th),2))/100)));  

    /* Parabolic field OK -  be aware of horizon */
    // Tchekhovskoy et al. (2008); Nakamura et al. (2018):
    // A[i][j] = 0.004607*pow(12,5/4)*pow(r,0.75)*( 1 - sqrt(pow(cos(th),2)) );
