...
//    Rout = 50. ;
 //   Rout = 1000. ;
    ...
    DTd = 10.0 ;	/* dumping frequency, in units of M */
...
//	ZSLOOP(0,N1-1,0,N2-1) {
    ZLOOP {
        ...
        if(r >= rin) {
            lnh = 
            -log(((pow(a,2) + (-2 + r)*r)*(pow(a,2)*pow(cth,2) 
				+ pow(r,2))*pow(sth,2))/(-(pow(l,2)*(-2 + r)*r) + r*pow(sth,2)*(-4*a*l + pow(a,2)*r + pow(r,3) 
				+ 2*pow(a,2)*pow(sth,2)) + pow(a,2)*pow(cth,2)*(-pow(l,2) + (pow(a,2) + pow(r,2))*pow(sth,2))))/2. 
			+ log(((pow(a,2) + (-2 + rin)*rin)*(pow(a,2)*pow(cthin,2) 
				+ pow(rin,2))*pow(sthin,2))/(-(pow(l,2)*(-2 + rin)*rin) + rin*pow(sthin,2)*(-4*a*l + pow(a,2)*rin 
				+ pow(rin,3) + 2*pow(a,2)*pow(sthin,2)) + pow(a,2)*pow(cthin,2)*(-pow(l,2) + (pow(a,2) 
					+ pow(rin,2))*pow(sthin,2))))/2.;
        }
        else
            lnh = 1. ;
        ...
        if(lnh < SMALL || r < rin|| cth > 0.7 || cth < -0.7) {
            rho = 1.e-7*RHOMIN ;
            u = 1.e-7*UUMIN ;
